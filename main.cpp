...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
    ...
    GLdouble radio = 0;
    Pelota *pelota = NULL;
    obstaculos = new Lista<Obstaculo*>();
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::GLScene()
{
        ...
        if (pelota != NULL){
                ...
                obstaculos->Primero();
                while (obstaculos->Actual()){
                        Obstaculo *aux = obstaculos->ValorActual();
                        glBegin(GL_POLYGON);
                                glVertex2d(aux->vertices[0]->x,aux->vertices[0]->y);
                                glVertex2d(aux->vertices[1]->x,aux->vertices[1]->y);
                                glVertex2d(aux->vertices[2]->x,aux->vertices[2]->y);
                        glEnd();
                        obstaculos->Siguiente();
                }
        }
        glFlush();
        SwapBuffers(hdc);
        ...
        if (inicio == true)
          {
                ...
                PV2D *v1 = new PV2D(ClientWidth *4,0);
                PV2D *v2 = new PV2D(ClientWidth,ClientHeight);
                PV2D *v3 = new PV2D(ClientWidth,- ClientHeight);

                Obstaculo *triangulo = new Triangulo(v1,v2,v3);
                obstaculos->InsertarPrincipio(triangulo);

                v1 = new PV2D(- ClientWidth *4 ,0);
                v2 = new PV2D(- ClientWidth,ClientHeight);
                v3 = new PV2D(- ClientWidth,- ClientHeight);

                triangulo = new Triangulo(v1,v2,v3);
                obstaculos->InsertarPrincipio(triangulo);


                v1 = new PV2D(0,ClientHeight *4);
                v2 = new PV2D(ClientWidth,ClientHeight);
                v3 = new PV2D(- ClientWidth, ClientHeight);
                triangulo = new Triangulo(v1,v2,v3);
                obstaculos->InsertarPrincipio(triangulo);
